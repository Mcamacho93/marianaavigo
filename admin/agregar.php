        ...
        if(isset($_POST['tipo'])){
            if ($_POST['tipo'] == 1){
                /*foreach($_POST as $name => $val){
                    ...
                    echo "<br>FILES: ".$_FILES['imgCat']['name'];
                }*/
                ...
                $nombreCategoria = $bd->escapestr($_POST['catNueva']);
                ...
                if($bd->rows($vc)>0)
                    ...
                    if($bd->query($queryCategoria)){
                        ...
                        if(exif_imagetype($_FILES['imgCat']['tmp_name']) != IMAGETYPE_JPEG && exif_imagetype($_FILES['imgCat']['tmp_name']) != IMAGETYPE_PNG){
                            echo "Solo se admiten formatos .jpg y .png";
                        }
                        else{
                            if(move_uploaded_file($_FILES['imgCat']['tmp_name'], $imagename)){
                                $update = "update category set img = '".$imgsql."' where categoryID = ".$idinsert."";
                                if($bd->query($update)){
                                    $arrayCat = array();
